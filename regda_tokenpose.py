...
def main(args: argparse.Namespace):
    ...
    backbone = models.get_tokenpose_L(istrain=True)
    #upsampling = Upsampling(backbone.out_features)
    upsampling = torch.nn.Identity()
    num_keypoints = train_source_dataset.num_keypoints
    model = RegDAPoseResNet(backbone, upsampling, 48, num_keypoints, num_head_layers=args.num_head_layers, finetune=True).to(device)
    ...
    if args.resume is None:
        if args.pretrain is None:
            ...
            pretrained_model = PoseResNet(backbone, upsampling, 48, num_keypoints, True).to(device) #32 small

...
def main(args: argparse.Namespace):
    ...
    optimizer_f = SGD([
        {'params': backbone.parameters(), 'lr': 0.15},
        {'params': upsampling.parameters(), 'lr': 0.15},
    ], lr=0.15, momentum=args.momentum, weight_decay=args.wd, nesterov=True)
    optimizer_h = SGD(model.head.parameters(), lr=1.5, momentum=args.momentum, weight_decay=args.wd, nesterov=True)
    optimizer_h_adv = SGD(model.head_adv.parameters(), lr=1.5, momentum=args.momentum, weight_decay=args.wd, nesterov=True)
    #lr_decay_function = lambda x: args.lr * (1. + args.lr_gamma * float(x)) ** (-args.lr_decay)
    lr_decay_function = lambda x: 0.15 * (1. + args.lr_gamma * float(x)) ** (-args.lr_decay)
